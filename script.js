...
// Returns the real visible part of an object-fit:contain image. This is the
// important part: the image element can be larger than the actual map drawing.
function getImageBox(el){
  const r=el.getBoundingClientRect();
  const nw=el.naturalWidth||1600,nh=el.naturalHeight||1400,ratio=nw/nh;
  let w=r.width,h=r.height;
  if(w/h>ratio)w=h*ratio;else h=w/ratio;
  return {left:r.left+(r.width-w)/2,top:r.top+(r.height-h)/2,width:w,height:h};
}
function getImagePoint(clientX,clientY){
  const box=getImageBox(map.classList.contains('relief-mode')?reliefMap:normalMap);
  const sx=(clientX-box.left)/box.width,sy=(clientY-box.top)/box.height;
  if(sx<0||sx>1||sy<0||sy>1)return null;
  return {sx,sy};
}
function placeGuess(sx,sy){
  const active=getActiveImage();
  const nw=active.naturalWidth||1600,nh=active.naturalHeight||1400,ratio=nw/nh;
  const cw=content.clientWidth,ch=content.clientHeight;
  let w=cw,h=ch;
  if(w/h>ratio)w=h*ratio;else h=w/ratio;
  const left=(cw-w)/2,top=(ch-h)/2;
  const m=document.createElement('span');m.className='guess-marker';m.style.left=`${left+sx*w}px`;m.style.top=`${top+sy*h}px`;guessLayer.appendChild(m);
}
...
function submitGuess(clientX,clientY){if(attempt>=3||!target)return;
  const point=getImagePoint(clientX,clientY);if(!point)return;
  const {sx,sy}=point;
